...
def encontrar_pelicula_mas_larga(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> dict:
    ...
    """
    peliculas = [p1, p2, p3, p4, p5]
    return max(peliculas, key=lambda peli: peli['duracion'])
...
def duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> str:
    ...
    """
    peliculas = [p1, p2, p3, p4, p5]
    duracion = sum(peli['duracion'] for peli in peliculas)
    promedio = duracion // len(peliculas)
    horas = promedio // 60
    minutos = promedio % 60
...
def encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict, anio: int) -> str:
    ...
    """
    peliculas = [p1, p2, p3, p4 ,p5]
    estrenos = [peli['nombre'] for peli in peliculas if peli['anio']==anio]
...
def cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> int:
    ...
    """
    peliculas = [p1, p2, p3, p4, p5]
    return sum(1 for peli in peliculas if peli['clasificacion']=='18+')
...
def reagendar_pelicula(peli:dict, nueva_hora: int, nuevo_dia: str, 
    ...
    for otra_peli in peliculas:
        if otra_peli['dia'] == nuevo_dia and otra_peli['hora'] == nueva_hora:
            return False
        
    if control_horario:
        if nuevo_dia not in ['domingo']:
            return False
...
def decidir_invitar(peli: dict, edad_invitado: int, autorizacion_padres: bool)->bool:
    ...
    if clasificacion == '18+':
        return edad_invitado >= 18 or autorizacion_padres
    elif clasificacion == '13+':
        return edad_invitado >= 13 or autorizacion_padres
